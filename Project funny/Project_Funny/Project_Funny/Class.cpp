...
void Class::InputList()
{
	...
	string ID = x.get_ID();
	string Lname = x.get_Lastname();
	string Fname = x.get_Firstname();
	for (int i = 1; i < 4; i++)
	{
		string semester = "HK" + to_string(i);
		vector<COURSE>COurse;
		LoadfiletxtCourse(COurse, semester, path);
		for (int j = 0; j < COurse.size(); j++)
		{
			string id_course = COurse[j].get_Id_Course();
			vector<CheckAttend_Point> lstcap;
			LoadFiletxtListOfCourse(lstcap, semester, id_course, path);
			CheckAttend_Point t(ID, Lname, Fname);
			//t.Input_Attend_Point();
			lstcap.push_back(t);
...
void Class::Remove()
{
	...
	for (int i = 1; i < 4; i++)
	{
		...
		for (int j = 0; j < COurse.size(); j++)
		{
			...
			{
				if (lstcap[i].get_id() == lst[remove1 - 1].get_ID())
				{
